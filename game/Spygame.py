    ...
    async def spyfall_game(self, ctx: discord.Interaction):
        ...
        async def start_callback(interaction):
            if interaction.user.id not in playersID:
                await interaction.response.send_message(
                    f"Minimal join dulu dek {interaction.user.name}-nyan",
                    ephemeral=True)
                return
            ...
            for player in playersID:
                ...
                if player == playersID[spyIndex]:
                    ...
                    embedMem = discord.Embed(
                        title=
                        f"You are NOT the spy! The Location is {location}",
                        description=
                        ...
                        if len(votes) < int(playerCount * 3 / 4):
                            ...
                            if flag > 1:
                                ...
                                async def spy_callback(interaction):
                                    if interaction.user.id != playersID[
                                            spyIndex]:
                                        await interaction.response.send_message(
                                ...
                                embedSpy = discord.Embed(
                                    ...
                                    color=0x330066)
                                spyMsg = await voteMsg.channel.send(
                                    embed=embedSpy, view=spyView)
                                ...
                                if checkSpyView:
                                    embedSpyNot = discord.Embed(
                                        ...
                                        color=0x330066)
                                    await spyMsg.edit(embed=embedSpyNot,
                                                      view=None)
        ...
        if checkView:
            embedEdit = discord.Embed(
                ...
                description=
                "Watashi udah nunggu lama tapi belum distart juga..",
                color=0x330066)
