...
class Anitrade(commands.Cog):
    ...
    async def anime_trading(self, ctx: discord.Interaction,
        ...
        async def add_callback(interaction):
            ...
            modaler = Modal(
                title="Add your anime character to the trading board",
                timeout=None)
            modaler.add_item(
                TextInput(
                ...
                if aniName in animeInven:
                    ...
                    if adderFind["animeCount"][animeIndex] < int(
                        ...
                        embedEdit.add_field(name=f"[ {member.name}'s Offer ]",
                                            value=f"```{edString2}```",
                                            inline=True)
                        await minteraction.response.edit_message(
                            embed=embedEdit)
        ...
        async def remove_callback(interaction):
            ...
            modaler = Modal(
                title="Remove anime character from the trading board",
                timeout=None)
            modaler.add_item(
                TextInput(
                    label='Anime Character Name',
                    ...
                    if namea in animeInven:
                        ...
                        newValues = {
                            "$set": {
                                ...
                                "allAni": mAll + 1,
                                stringIndex:
                                animeCounting[animeIndex] + numbera
                            }
                        }
        ...
        async def cancel_callback(interaction):
            ...
            embedEnd = discord.Embed(
                ...
                description=
                f"Cause : Cancelled by {interaction.user.name}-nyan",
                color=0xff69b4)
