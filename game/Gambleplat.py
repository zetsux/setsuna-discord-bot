...
class Gambleplat(commands.Cog):
    ...
    @app_commands.command(
        name='gambleplat',
        description='Gamble your platina with approximately 50/50 odds of getting double or losing all'
    )
    @app_commands.describe(number='Number of platina to gamble')
    async def platina_gamble(self, ctx: discord.Interaction, number: int):
        if number <= 0:
