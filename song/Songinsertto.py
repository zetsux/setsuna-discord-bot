...
class Songinsertto(commands.Cog):
  ...
  @app_commands.command(name='songinsertto', description='Insert track/album/playlist from spotify/youtube to the inserted position of the queue')
  @app_commands.checks.has_any_role('Encoder Magang', 'Owner')
  @app_commands.describe(search="The link or key to search for", position="The position to insert the track in queue")
  async def song_to(self, ctx: discord.Interaction, *, search: str, position: int):
    ...
    if not ctx.user.voice:
      ...
      await ctx.followup.send(
            f'Hmph {ctx.user.name}-nyan, gajelas ah masa positionnya gitu, mulai dari 1 dong..',
            ephemeral=True)
      return
    ...
    if decoded:
        if decoded['type'] is spotify.SpotifySearchType.track:
            ...
            if vc.queue.is_empty and not vc.is_playing():
                ...
                Songlist.songList.insert(position, [track, ctx.user])
                embedVar = discord.Embed(
        ...
        if 'youtube.com/playlist' in search:
            ...
            if vc.queue.is_empty and not vc.is_playing():
                ...
                Songlist.songList.insert(position, [searchYt, ctx.user])
                embedVar = discord.Embed(
                    title=f'Queueing to position {position + 1} :',
...
async def setup(bot):
  await bot.add_cog(Songinsertto(bot))