...
def arrangelb(idUser):
    ...
    for i in lbBoard:
        ...
        if dWin > iUser["win"] or (
                dWin == iUser["win"]
                and dLose < iUser["lose"]) or (dWin == iUser["win"]
                                               and dLose == iUser["lose"]
                                               and dDraw > iUser["lose"]):
            break
    ...
    async def pokemon_duel(self, ctx: discord.Interaction):
        ...
        async def battle_callback(interaction):
            ...
            embedDel = discord.Embed(
                title=f'[ PokeDuel Challenge is underway! ]',
            ...
            response0 = requests.get(
                f'https://pokeapi.co/api/v2/pokemon/{poke0.lower()}')
            data0 = response0.json()
            response1 = requests.get(
                f'https://pokeapi.co/api/v2/pokemon/{poke1.lower()}')
            data1 = response1.json()
            hp = [
                int(
                    int(data0["stats"][0]["base_stat"]) *
                    ((((partLevel0 * 2) - 1) + 19) / 20)),
                int(
                    int(data1["stats"][0]["base_stat"]) *
                    ((((partLevel1 * 2) - 1) + 19) / 20))
            ]
            atk = [
                int(
                    int(data0["stats"][1]["base_stat"]) *
                    ((((partLevel0 * 2) - 1) + 19) / 20)),
                int(
                    int(data1["stats"][1]["base_stat"]) *
                    ((((partLevel1 * 2) - 1) + 19) / 20))
            ]
            spatk = [
                int(
                    int(data0["stats"][3]['base_stat']) *
                    ((((partLevel0 * 2) - 1) + 19) / 20)),
                int(
                    int(data1["stats"][3]['base_stat']) *
                    ((((partLevel1 * 2) - 1) + 19) / 20))
            ]
            defense = [
                int(
                    int(data0["stats"][2]['base_stat']) *
                    ((((partLevel0 * 2) - 1) + 19) / 20)),
                int(
                    int(data1["stats"][2]['base_stat']) *
                    ((((partLevel1 * 2) - 1) + 19) / 20))
            ]
            spdef = [
                int(
                    int(data0["stats"][4]['base_stat']) *
                    ((((partLevel0 * 2) - 1) + 19) / 20)),
                int(
                    int(data1["stats"][4]['base_stat']) *
                    ((((partLevel1 * 2) - 1) + 19) / 20))
            ]
            speed = [
                int(
                    int(data0["stats"][5]['base_stat']) *
                    ((((partLevel0 * 2) - 1) + 19) / 20)),
                int(
                    int(data1["stats"][5]['base_stat']) *
                    ((((partLevel1 * 2) - 1) + 19) / 20))
            ]
            element = [[
                t['type']['name'].capitalize() for t in data0['types']
            ], [t['type']['name'].capitalize() for t in data1['types']]]
            eleString = [
                ', '.join(
                    [t['type']['name'].capitalize() for t in data0['types']]),
                ', '.join(
                ...
                if dinteraction.user.id == turn:
                    ...
                    if len(element[dmgdIndex]) == 1:
                        if len(element[atkerIndex]) == 1:
                            dmgMul = int(typeEff[element[atkerIndex][0]][
                                indexEff[element[dmgdIndex][0]]]) / 2
                            atkElement = element[atkerIndex][0]

                        elif len(element[atkerIndex]) == 2:
                            mul1 = int(typeEff[element[atkerIndex][0]][
                                indexEff[element[dmgdIndex][0]]]) / 2
                            mul2 = int(typeEff[element[atkerIndex][1]][
                                indexEff[element[dmgdIndex][0]]]) / 2
                        ...
                        if len(element[atkerIndex]) == 1:
                            dmgMul = (int(typeEff[element[atkerIndex][0]][
                                indexEff[element[dmgdIndex][0]]]) / 2) * (
                                    int(typeEff[element[atkerIndex][0]][
                                        indexEff[element[dmgdIndex][1]]]) / 2)
                            atkElement = element[atkerIndex][0]

                        elif len(element[atkerIndex]) == 2:
                            mul1 = (int(typeEff[element[atkerIndex][0]][
                                indexEff[element[dmgdIndex][0]]]) /
                    ...
                    damageDealt = int(
                        ((70 *
                          (atk[atkerIndex] *
                           (boosta[atkerIndex] / 2)) / defense[dmgdIndex]) /
                         7.5) * ((10 + int(partLevel1)) / 10) * 1.5 * dmgMul)
            ...
            async def spatk_callback(dinteraction):
                ...
                if dinteraction.user.id == turn:
                    ...
                    damageDealt = int(
                        ((70 * (spatk[atkerIndex] *
                                (boosts[atkerIndex] / 2)) / spdef[dmgdIndex]) /
                         7.5) * ((10 + int(partLevel1)) / 10) * 1.5 * dmgMul)
