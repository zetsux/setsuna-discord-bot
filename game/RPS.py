...
class RPS(commands.Cog):
    ...
    @app_commands.command(name='rps',
                          description='Battle RPS against a target')
    @app_commands.describe(member="The member to battle with")
    async def rps_battle(self, ctx: discord.Interaction,
                         member: discord.Member):
        if member.bot:
            await ctx.response.send_message(
                f'Masa nantang bewan sama bot sih {ctx.user.name}-nyan, aneh banget deh',
                ephemeral=True)
            return
        ...
        player2 = "kosong"
        select = Select(
            placeholder="Choose your hand!!",
            options=[
                discord.SelectOption(label="The Resilient Rock",
                                     emoji="🪨",
                                     value="r",
